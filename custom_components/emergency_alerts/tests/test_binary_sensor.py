...
async def test_simple_trigger_setup(hass: HomeAssistant, mock_config_entry: MockConfigEntry):
...
async def test_action_calls(hass: HomeAssistant):
    ...
    on_triggered = [{"service": "notify.notify", "data": {"message": "Alert triggered!"}}]
    ...
    with patch.object(hass.services, 'async_call', new_callable=AsyncMock) as mock_call:
        ...
        mock_call.assert_called_with("notify", "notify", {"message": "Alert triggered!"}, blocking=False)
        ...
        mock_call.assert_called_with("notify", "notify", {"message": "Alert cleared!"}, blocking=False)
